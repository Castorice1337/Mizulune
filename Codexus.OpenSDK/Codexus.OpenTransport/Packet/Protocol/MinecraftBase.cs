    ...
    public void ApplyTo(MinecraftRegistry registry, RegistryScope scope)
    {
        registry.Builder(scope)
            ...
            .Attach<C2SHandshake>((context, packet) =>
            {
                ...
                if (session.Request.RequiredProtocolVersion is { } required
                    && packet.ProtocolVersion != required)
                {
                    context.Cancel();
                    EventBus.Instance.Publish(new EventProtocolRejected(
                        context.Transport,
                        (int)packet.ProtocolVersion,
                        (int)required));
                    _ = session.Local.CloseAsync();
                    return;
                }
