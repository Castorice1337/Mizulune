...
public class PlayerPatch {
    ...
    public static void onIsStayingOnGroundSurface(Player player, CallbackInfo callbackInfo) {
        HookDecision<Boolean> decision = EntityHookCallbacks.onIsStayingOnGroundSurface(player);
        if (decision.handled()) {
    ...
    public static float onDieGetYRot(Player player, DamageSource source, Invocation<Player, Float> original) throws Exception {
        return PlayerHookCallbacks.actionYaw(player, original.call());
    }
    ...
    @WrapInvoke(method = "attack", desc = "(Lnet/minecraft/world/entity/Entity;)V", target = "net/minecraft/world/entity/player/Player/getYRot", targetDesc = "()F")
    ...
    public static void onAttackPre(Player player, Entity target, CallbackInfo callbackInfo) {
        PlayerHookCallbacks.onAttack(player, target, false);
    }
    ...
    public static void onAttackPost(Player player, Entity target, CallbackInfo callbackInfo) {
        PlayerHookCallbacks.onAttack(player, target, true);
    }
}
