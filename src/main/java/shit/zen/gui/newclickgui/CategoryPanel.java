...
extends UIElement {
    ...
    /**
     * Rebuilds the cached module elements after ModuleManager initialization.
     * NewClickGui is a static screen, so its panels may be constructed before
     * the first tick has registered the modules.
     */
    public void refreshModules() {
        this.moduleElements.clear();
        if (ZenClient.getInstance() != null && ZenClient.getInstance().getModuleManager() != null) {
            for (Module module : ZenClient.getInstance().getModuleManager().getModulesByCategory(this.category)) {
