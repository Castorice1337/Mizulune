...
public final class ValueTreeElementRenderer {
    ...
    private float renderLeaf(CategoryPanel parentPanel, PoseStack poseStack, Value<?> value, float x, float y,
        ...
        if (value instanceof ActionValue actionValue) {
            return this.renderAction(poseStack, actionValue, x, y, width, mouseX, mouseY, alpha);
        }
        if (this.isDropdownValue(value)) {
            return this.renderDropdownValue(
                poseStack, value, value.getDisplayName(), x, y, width, mouseX, mouseY, alpha, false);
        }
    ...
    private float renderAction(PoseStack poseStack, ActionValue value, float x, float y, float width,
                               int mouseX, int mouseY, float alpha) {
        float buttonWidth = 48.0f;
        float buttonX = x + width - buttonWidth - SIDE_PADDING;
        boolean hovered = CursorUtil.isInBounds(
            mouseX, mouseY, buttonX, y + 3.0f, buttonWidth, 12.0f);
        String name = this.clip(value.getDisplayName(), FontStore.AXIFORMA_REGULAR_14, width - 64.0f);
        FontStore.AXIFORMA_REGULAR_14.drawString(poseStack, name, x + SIDE_PADDING,
            y + (BOOLEAN_HEIGHT - FontStore.AXIFORMA_REGULAR_14.getFontHeight()) / 2.0f,
            Argb.withAlpha(-1, alpha * 0.8f));
        if (hovered) {
            RenderUtil.drawRoundedRect(poseStack, buttonX, y + 3.0f, buttonWidth, 12.0f, 3.0f,
                Argb.withAlpha(-1, alpha * 0.12f));
        }
        this.renderSmallPill(
            poseStack, value.getActionLabel(), buttonX, y + 3.0f, buttonWidth, 12.0f, alpha);
        return BOOLEAN_HEIGHT;
    }
    ...
    private float renderDropdownValue(PoseStack poseStack, Value<?> value, String displayName, float x, float y,
        ...
        String current = this.clip(this.currentDropdownLabel(value), FontStore.AXIFORMA_BOLD_13, dropdownWidth - 22.0f);
    ...
    private boolean onLeafClick(Value<?> value, float x, float y, float width, int mouseX, int mouseY, int button) {
        ...
        if (value.getType() != ValueType.TEXT || this.isDropdownValue(value)) {
            this.blurText();
        }
        if (value instanceof ActionValue actionValue) {
            float buttonWidth = 48.0f;
            if (button == 0 && CursorUtil.isInBounds(
                    mouseX, mouseY, x + width - buttonWidth - SIDE_PADDING, y + 3.0f, buttonWidth, 12.0f)) {
                actionValue.trigger();
                return true;
            }
            return false;
        ...
        if (this.isDropdownValue(value)) {
            return this.onDropdownClick(value, x, y, width, mouseX, mouseY, button);
        }
        return switch (value.getType()) {
            case BOOLEAN -> {
    ...
    private boolean onDropdownClick(Value<?> value, float x, float y, float width, int mouseX, int mouseY, int button) {
        ...
        if (CursorUtil.isInBounds(mouseX, mouseY, x + SIDE_PADDING, dropdownY, dropdownWidth, DROPDOWN_ITEM_HEIGHT)) {
            this.dropdownOpen.put(value, !options.isEmpty() && !this.isDropdownOpen(value));
            return true;
        }
        return false;
    }
    ...
    private boolean isDropdownValue(Value<?> value) {
        return value.getType() == ValueType.ENUM
            || Boolean.TRUE.equals(value.getMetadata().get("dropdown"));
    }
    ...
    private String currentDropdownLabel(Value<?> value) {
        ...
        if (!current.isBlank()) return this.optionLabel(value, current);
