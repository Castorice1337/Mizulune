...
public final class ValueRendererRegistry {
    ...
    private int renderLeaf(GuiGraphics guiGraphics, Value<?> value, int x, int y, int width,
        ...
        if (value instanceof ActionValue actionValue) {
            this.drawPill(guiGraphics, actionValue.getActionLabel(),
                x + width - Math.round(96.0f * scale), y + Math.round(5.0f * scale),
                Math.round(90.0f * scale), Math.round(14.0f * scale), alpha, scale);
        } else if (this.isDropdownValue(value)) {
            this.drawPill(guiGraphics, this.currentDropdownLabel(value) + "  v",
                x + width - Math.round(116.0f * scale), y + Math.round(5.0f * scale),
                Math.round(110.0f * scale), Math.round(14.0f * scale), alpha, scale);
        } else {
        ...
        if (this.isDropdownValue(value) && this.isDropdownOpen(value)) {
            this.renderDropdownOptions(
                guiGraphics, value, x, y + baseHeight, width, mouseX, mouseY, alpha, scale);
        }
    ...
    private boolean onLeafClick(Value<?> value, int x, int y, int width, int mouseX, int mouseY, int button, float scale) {
        ...
        if (value instanceof ActionValue actionValue) {
            int pillX = x + width - Math.round(96.0f * scale);
            int pillY = y + Math.round(5.0f * scale);
            if (button == 0 && inBounds(mouseX, mouseY, pillX, pillY,
                    Math.round(90.0f * scale), Math.round(14.0f * scale))) {
                actionValue.trigger();
                this.addToast(actionValue.getDisplayName());
                return true;
            }
            return false;
        }
        if (this.isDropdownValue(value)) {
            return this.onDropdownClick(value, x, y, width, mouseX, mouseY, button, scale);
        }
        switch (value.getType()) {
            case BOOLEAN -> {
    ...
    private void cycleEnum(Value<String> value, int direction) {
        ...
        for (int i = 0; i < options.size(); i++) {
            if (options.get(i).equalsIgnoreCase(String.valueOf(value.getValue()))) {
                current = i;
                break;
            }
        }
        int next = Math.floorMod(current + direction, options.size());
        value.setValue(options.get(next));
    }
    ...
    private int leafHeight(Value<?> value, float scale) {
        if (value.getType() == ValueType.MULTI_ENUM) {
            return Math.round(22.0f * scale) + Math.round(value.getOptions().size() * 18.0f * scale);
        }
        if (this.isDropdownValue(value) && this.isDropdownOpen(value)) {
            return this.leafBaseHeight(scale)
                + Math.round(value.getOptions().size() * 18.0f * scale);
        }
    ...
    private boolean onDropdownClick(Value<?> value, int x, int y, int width,
                                    int mouseX, int mouseY, int button, float scale) {
        if (button != 0 && button != 1) return false;
        List<String> options = value.getOptions();
        int rowHeight = Math.round(18.0f * scale);
        int optionY = y + this.leafBaseHeight(scale);
        if (this.isDropdownOpen(value)) {
            for (String option : options) {
                if (inBounds(mouseX, mouseY, x, optionY, width, rowHeight)) {
                    @SuppressWarnings("unchecked")
        ...
        int pillX = x + width - Math.round(116.0f * scale);
        int pillY = y + Math.round(5.0f * scale);
        if (inBounds(mouseX, mouseY, pillX, pillY,
                Math.round(110.0f * scale), Math.round(14.0f * scale))) {
            boolean open = !options.isEmpty() && !this.isDropdownOpen(value);
            this.dropdownOpen.replaceAll((ignored, previous) -> false);
            this.dropdownOpen.put(value, open);
            if (options.isEmpty()) this.addToast(this.currentDropdownLabel(value));
            return true;
        }
        return false;
    }

    private void renderDropdownOptions(GuiGraphics guiGraphics, Value<?> value, int x, int y, int width,
                                       int mouseX, int mouseY, float alpha, float scale) {
        int rowHeight = Math.round(18.0f * scale);
        FontRenderer font = FontPresets.axiformaRegular(12.0f * scale);
        for (String option : value.getOptions()) {
            boolean hovered = inBounds(mouseX, mouseY, x, y, width, rowHeight);
            int background = hovered ? new Color(255, 255, 255, 24).getRGB()
                : new Color(255, 255, 255, 12).getRGB();
            RenderUtil.drawRoundedRect(guiGraphics.pose(), x, y, width, rowHeight,
                3.0f * scale, this.applyAlpha(background, alpha));
            float textY = y + rowHeight / 2.0f - font.getMetrics().capHeight() / 2.0f;
            this.drawText(guiGraphics, option, x + Math.round(8.0f * scale), textY, font,
                this.applyAlpha(option.equalsIgnoreCase(String.valueOf(value.getValue()))
                    ? 0xFFEAF7FF : 0xFFBBBBBB, alpha));
            y += rowHeight;
        }
    }

    private boolean isDropdownValue(Value<?> value) {
        return Boolean.TRUE.equals(value.getMetadata().get("dropdown"));
    }
    ...
    private String currentDropdownLabel(Value<?> value) {
        String current = value.getValue() == null ? "" : String.valueOf(value.getValue());
        if (!current.isBlank()) return current;
        Object emptyLabel = value.getMetadata().get("emptyOptionLabel");
        return emptyLabel == null ? "None" : String.valueOf(emptyLabel);
    }
