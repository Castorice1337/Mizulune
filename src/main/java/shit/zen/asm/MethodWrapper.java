...
public final class MethodWrapper {
    ...
    public static MethodWrapper getInstance(String classOwner, String methodName, String methodDesc) throws Exception {
        ...
        MethodKey key = new MethodKey(clazz, methodName, methodDesc);
        MethodHandle cached = CACHE.get(key);
        if (cached != null) {
            return new MethodWrapper(cached);
        }
        MethodHandle handle = lookup(clazz, methodName, methodDesc, new HashSet<>());
        if (handle == null) {
    ...
    private static MethodHandle lookup(
            ...
            String methodName,
            String methodDesc,
            Set<Class<?>> visited) throws Exception {
        ...
        MethodHandle inherited = lookup(clazz.getSuperclass(), methodName, methodDesc, visited);
        if (inherited != null) {
            return inherited;
        }
        for (Class<?> implementedInterface : clazz.getInterfaces()) {
            inherited = lookup(implementedInterface, methodName, methodDesc, visited);
            if (inherited != null) {
                return inherited;
            }
        }
    ...
    private record MethodKey(Class<?> owner, String name, String descriptor) {
    }
}
