...
public final class PlayerPositionHold extends ClientBase {
    ...
    public enum MovePacketPolicy {
        POSITION_ONLY,
        BLOCK_ALL;

        boolean shouldCancel(boolean hasPosition) {
            return this == BLOCK_ALL || hasPosition;
        }
    }
    ...
    public static int hold(Object owner, int ticks) {
        return hold(owner, ticks, MovePacketPolicy.POSITION_ONLY);
    }

    public static int hold(Object owner, int ticks, MovePacketPolicy policy) {
        ...
        synchronized (HOLDS) {
            ...
            current.policy = policy == null ? MovePacketPolicy.POSITION_ONLY : policy;
    ...
    public static int extend(Object owner, int ticks) {
        return extend(owner, ticks, null);
    }

    public static int extend(Object owner, int ticks, MovePacketPolicy policy) {
        if (owner == null || mc.player == null || ticks <= 0) {
            return 0;
        }
        synchronized (HOLDS) {
    ...
    public static void holdUntilRelease(Object owner) {
        holdUntilRelease(owner, MovePacketPolicy.POSITION_ONLY);
    }

    public static void holdUntilRelease(Object owner, MovePacketPolicy policy) {
    ...
    public void onPacket(PacketEvent event) {
        ...
        if (!state.policy.shouldCancel(movePacket.hasPosition())) {
            emit(state.owner, movePacket.hasRotation() ? "rot-pass" : "status-pass");
            return;
        }
        if (state.policy == MovePacketPolicy.BLOCK_ALL) {
            emit(state.owner, "move-block:" + packetKind(movePacket));
    ...
    private static String packetKind(ServerboundMovePlayerPacket packet) {
        if (packet.hasPosition()) {
            return packet.hasRotation() ? "posrot" : "pos";
        }
        return packet.hasRotation() ? "rot" : "status";
    }
    ...
    private static final class HoldState {
        ...
        private MovePacketPolicy policy = MovePacketPolicy.POSITION_ONLY;
