...
public final class ReflectionUtil {
    ...
    /**
     * Keeps vanilla's last-sent hotbar cache aligned with slot packets emitted outside
     * {@code MultiPlayerGameMode.ensureHasSentCarriedItem()}.
     */
    public static void setCarriedIndex(MultiPlayerGameMode gameMode, int slot) {
        if (gameMode == null || slot < 0 || slot > 8) return;
        unsafe.putIntVolatile(
                gameMode,
                getFieldOffset(MultiPlayerGameMode.class, "carriedIndex"),
                slot);
    }

    public static int getCarriedIndex(MultiPlayerGameMode gameMode) {
        if (gameMode == null) return -1;
        return unsafe.getIntVolatile(
                gameMode,
                getFieldOffset(MultiPlayerGameMode.class, "carriedIndex"));
    }

    public static String getMappedFieldName(Class<?> clazz, String fieldName) {
        return ClientPlatforms.current().remapField(clazz.getName().replace('.', '/'), fieldName);
    }

    public static String getMappedMethodName(Class<?> clazz, String methodName, String descriptor) {
        return ClientPlatforms.current().remapMethod(
                clazz.getName().replace('.', '/'), methodName, descriptor);
    }
    ...
    private static Field resolveField(Class<?> clazz, String name) throws NoSuchFieldException {
        ...
        for (Class<?> c = clazz; c != null; c = c.getSuperclass()) {
            String srg = ClientPlatforms.current().remapField(
