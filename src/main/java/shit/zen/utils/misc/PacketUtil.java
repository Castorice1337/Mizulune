...
extends ClientBase {
    ...
    /** Marks an infrastructure packet to bypass both cancellable outbound event layers once. */
    public static void prepareDirectSend(Packet<?> packet) {
    ...
    /** Removes an unused direct-send marker after a lifecycle cancellation or dispatch failure. */
    public static void cancelDirectSend(Packet<?> packet) {
        if (packet == null) return;
        synchronized (preparedBufferedPackets) {
            preparedBufferedPackets.remove(packet);
        }
    }
