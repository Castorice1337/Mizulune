...
extends AntiKBMode {
    ...
    private void restoreMovementKeys() {
        mc.options.keyUp.setDown(InputConstants.isKeyDown(mc.getWindow().getWindow(), mc.options.keyUp.key.getValue()));
        mc.options.keyDown.setDown(InputConstants.isKeyDown(mc.getWindow().getWindow(), mc.options.keyDown.key.getValue()));
        mc.options.keyLeft.setDown(InputConstants.isKeyDown(mc.getWindow().getWindow(), mc.options.keyLeft.key.getValue()));
        mc.options.keyRight.setDown(InputConstants.isKeyDown(mc.getWindow().getWindow(), mc.options.keyRight.key.getValue()));
    }
    ...
    public void onGameTick(GameTickEvent gameTickEvent) {
        if (this.knockbackPacket != null && !AntiKB.INSTANCE.movementOverride.getValue() && mc.player != null) {
            if (mc.player.hurtTime > 6 && !mc.options.keyJump.isDown()) {
                ...
                boolean jumpKeyDown = InputConstants.isKeyDown(mc.getWindow().getWindow(), mc.options.keyJump.key.getValue());
