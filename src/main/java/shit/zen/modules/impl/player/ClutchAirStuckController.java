...
final class ClutchAirStuckController extends ClientBase implements PlayerPositionHold.DebugSink {
    ...
    private Vec3 rescueMotion = Vec3.ZERO;
    private AABB rescueAnchor;
    ...
    void update(UpdateInput input) {
        ...
        if (!this.active) {
            ...
            this.active = true;
            this.windowTicks = 1;
            this.rescueMotion = mc.player.getDeltaMovement();
            this.captureHold();
    ...
    Vec3 planningMotion(Vec3 fallback) {
        if (this.active && this.rescueMotion != null) {
            return this.rescueMotion;
        }
        return fallback == null ? Vec3.ZERO : fallback;
    }

    boolean intersectsAnchor(AABB box) {
        return this.active
                && this.rescueAnchor != null
                && box != null
                && this.rescueAnchor.intersects(box);
    }
    ...
    private void captureHold() {
        this.rescueAnchor = mc.player == null ? null : mc.player.getBoundingBox();
        PlayerPositionHold.hold(this, HOLD_TICKS, PlayerPositionHold.MovePacketPolicy.BLOCK_ALL);
        this.debug.accept("air-stuck:anchor-capture hold=" + this.remainingHoldTicks());
    }

    private void extendHold() {
        PlayerPositionHold.extend(this, HOLD_TICKS, PlayerPositionHold.MovePacketPolicy.BLOCK_ALL);
    ...
    private boolean isConcreteTarget(
        ...
        AABB targetBox = new AABB(target.placedBlockPos());
        if (mc.player.getBoundingBox().intersects(targetBox) || this.intersectsAnchor(targetBox)) {
            return false;
        }
