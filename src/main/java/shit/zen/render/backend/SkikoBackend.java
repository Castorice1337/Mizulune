...
public final class SkikoBackend implements RenderBackend {
    ...
    private static final long MIN_GPU_CACHE_BYTES = 64L * MEBIBYTE;
    private static final long MAX_GPU_CACHE_BYTES = 192L * MEBIBYTE;
    private static final long GPU_CACHE_FRAME_COPIES = 6L;
    ...
    private long gpuCacheLimitBytes = -1L;
    ...
    public String debugSummary() {
        return "fbo=" + this.currentFbo
                ...
                + " gpuCacheMiB=" + (this.gpuCacheLimitBytes / MEBIBYTE)
    ...
    private void ensureSurface(int fbo, int width, int height) {
        this.configureGpuCacheLimit(width, height);
    ...
    /**
     * Skia keeps released render textures in its GPU resource cache. A full-size backdrop
     * snapshot is produced every frame, so leaving the cache at the driver default can exhaust
     * VRAM when another renderer (notably Sodium or another game) is active. Keep enough room for
     * the framebuffer, backdrop and temporary effect surfaces while placing a hard upper bound on
     * retained textures.
     */
    private void configureGpuCacheLimit(int width, int height) {
        if (this.directContext == null) {
            return;
        }
        long pixelBytes = Math.max(1L, (long) width)
                * Math.max(1L, (long) height)
                * Integer.BYTES;
        long requestedBytes = pixelBytes * GPU_CACHE_FRAME_COPIES;
        long boundedBytes = Math.max(MIN_GPU_CACHE_BYTES, Math.min(MAX_GPU_CACHE_BYTES, requestedBytes));
        if (boundedBytes == this.gpuCacheLimitBytes) {
            return;
        }
        this.directContext.setResourceCacheLimit(boundedBytes);
        this.gpuCacheLimitBytes = boundedBytes;
    }
