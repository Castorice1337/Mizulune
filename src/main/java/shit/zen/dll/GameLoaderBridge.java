...
public final class GameLoaderBridge {
    ...
    static final String SYNC_TOKEN_CLASS_NAME = "com.heypixel.heypixelmod.SyncToken";
    static final String SYNC_TOKEN_INTERNAL_NAME = "com/heypixel/heypixelmod/SyncToken";
    ...
    public static void load(String jarPath, ClassLoader gameLoader) throws Throwable {
        ...
        if (removeExactAbiClassIfVisible(
                pendingClasses,
                className -> isExactSyncTokenAbiVisible(className, gameLoader))) {
            LOGGER.info("Using game-loader-visible exact ABI class {}", SYNC_TOKEN_CLASS_NAME);
        }
    ...
    static boolean removeExactAbiClassIfVisible(
            LinkedHashMap<String, byte[]> pending,
            Predicate<String> visibility) {
        if (!pending.containsKey(SYNC_TOKEN_CLASS_NAME)
                || !visibility.test(SYNC_TOKEN_CLASS_NAME)) {
            return false;
        }
        pending.remove(SYNC_TOKEN_CLASS_NAME);
        return true;
    }

    static boolean isExactSyncTokenAbiVisible(String className, ClassLoader loader) {
        final Class<?> type;
        try {
        ...
        try {
            ...
            if (!exact) throw new IllegalStateException("visible exact ABI class mismatch");
            return true;
        } catch (ReflectiveOperationException error) {
            throw new IllegalStateException("visible exact ABI class mismatch");
        }
    }
     ...
     */
    static Map<String, String> buildRuntimeTypeMap(
        ...
        for (String dotted : classNames) {
            ...
            if (isExactAbiKeep(internal)) continue;
    ...
    static boolean isExactAbiKeep(String internal) {
        return SYNC_TOKEN_INTERNAL_NAME.equals(internal);
    }
     ...
     */
    static LinkedHashMap<String, byte[]> applyRuntimeObfuscation(
            LinkedHashMap<String, byte[]> pending,
        ...
        for (Map.Entry<String, String> e : typeMap.entrySet()) {
            if (isExactAbiKeep(e.getKey())) continue;
        ...
        Remapper remapper = new Remapper() {
            ...
            public String map(String internalName) {
                if (isExactAbiKeep(internalName)) return internalName;
        ...
        for (Map.Entry<String, byte[]> entry : pending.entrySet()) {
            ...
            if (isExactAbiKeep(oldInternal)) {
                result.put(oldDotted, oldBytes);
                continue;
            }
