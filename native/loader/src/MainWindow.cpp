...
MainWindow::MainWindow(QWidget* parent)
        : QMainWindow(parent),
          ...
          backendBridge_(new BackendProcessBridge(this)) {
    ...
    connect(backendBridge_, &BackendProcessBridge::responseReady, this, [this](const QJsonObject& response) {
        postEvent(QStringLiteral("backend.response"), response);
    });
    connect(backendBridge_, &BackendProcessBridge::eventReady, this, [this](const QJsonObject& event) {
        const QString name = event.value(QStringLiteral("event")).toString();
        if (name == QStringLiteral("game.started")) backendOwnsGame_ = true;
        if (name == QStringLiteral("game.stopped")) backendOwnsGame_ = false;
        postEvent(QStringLiteral("backend.event"), event);
    });
    connect(backendBridge_, &BackendProcessBridge::hostUnavailable, this, [this](const QString& error) {
        postEvent(QStringLiteral("backend.event"), {
...
void MainWindow::handleWebMessage(const QString& jsonText) {
    ...
    if (type == QStringLiteral("ready")) {
    ...
    } else if (type == QStringLiteral("backend.request")) {
        static const QSet<QString> allowed{
            QStringLiteral("host.status"),
            QStringLiteral("host.shutdown"),
            QStringLiteral("account.list"),
            QStringLiteral("account.available"),
            QStringLiteral("account.captcha.begin"),
            QStringLiteral("account.captcha.submit"),
            QStringLiteral("account.captcha.auto"),
            QStringLiteral("account.save"),
            QStringLiteral("account.update"),
            QStringLiteral("account.login"),
            QStringLiteral("account.switch"),
            QStringLiteral("account.delete"),
            QStringLiteral("server.list"),
            QStringLiteral("server.detail"),
            QStringLiteral("server.roles"),
            QStringLiteral("server.role.create"),
            QStringLiteral("session.prepare"),
            QStringLiteral("launch.start"),
            QStringLiteral("launch.list"),
            QStringLiteral("launch.stop"),
            QStringLiteral("proxy.start"),
            QStringLiteral("proxy.list"),
        ...
        if (id.isEmpty() || !allowed.contains(method)) {
            postEvent(QStringLiteral("backend.response"), {
                {QStringLiteral("id"), id},
                {QStringLiteral("ok"), false},
                {QStringLiteral("error"), QJsonObject{
                    {QStringLiteral("code"), QStringLiteral("method_not_allowed")},
                    {QStringLiteral("message"), QStringLiteral("Invalid backend request.")}
                }}
            });
            return;
        }
        backendBridge_->request(method,
...
void MainWindow::startInjection(const QJsonObject& payload) {
    ...
    if (!backendBridge_->isRunning() || session.value(QStringLiteral("gameId")).toString().isEmpty()
            || session.value(QStringLiteral("roleName")).toString().isEmpty()) {
        performInjection(pid, title);
        return;
    }

    backendBridge_->request(QStringLiteral("host.status"), {},
        [this, pid, title, session](const QJsonObject& response) {
            if (!response.value(QStringLiteral("ok")).toBool()) {
                ...
                postEvent(QStringLiteral("inject.finished"), {
                    ...
                    {QStringLiteral("error"), error.value(QStringLiteral("message")).toString(
                        QStringLiteral("Failed to query the Fantnel session."))}
                });
                return;
            }
            ...
            backendBridge_->request(QStringLiteral("session.prepare"), session,
                [this, pid, title](const QJsonObject& prepareResponse) {
                    if (!prepareResponse.value(QStringLiteral("ok")).toBool()) {
                        ...
                        postEvent(QStringLiteral("inject.finished"), {
                            {QStringLiteral("ok"), false},
                            {QStringLiteral("error"), error.value(QStringLiteral("message")).toString(
                                QStringLiteral("Failed to prepare the Fantnel session."))}
                        });
                        return;
                    }
                    performInjection(pid, title);
...
void MainWindow::performInjection(unsigned long pid, const QString& title) {
    ...
    std::thread([this, pid]() {
        ...
        QMetaObject::invokeMethod(this, [this, qError]() {
            ...
            if (ok && ProfileStore::load().closeAfterInjection && !backendOwnsGame_) {
                playExitThenQuit();
            } else {
                sendInstances();
            }
