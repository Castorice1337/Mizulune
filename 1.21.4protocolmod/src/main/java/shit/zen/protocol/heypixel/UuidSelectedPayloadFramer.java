...
public final class UuidSelectedPayloadFramer {
    ...
    private static byte[] reverseFourByteBlocks(byte[] bytes) {
        byte[] out = new byte[bytes.length];
        int completeBytes = bytes.length - bytes.length % 4;
        int cursor = 0;
        for (int offset = completeBytes - 4; offset >= 0; offset -= 4) {
            System.arraycopy(bytes, offset, out, cursor, 4);
            cursor += 4;
        }
        // The official layout reverses only complete dwords; a trailing partial block stays last.
        System.arraycopy(bytes, completeBytes, out, cursor, bytes.length - completeBytes);
        return out;
    }
