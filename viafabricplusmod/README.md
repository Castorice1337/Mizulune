# ViaFabricPlus OpenZen 766

This directory is the complete ViaFabricPlus 2.8.7 source fork for Fabric
1.20.1 / Java 17. It retains the upstream multiplayer version selector,
per-server forcing, settings screens, legacy/Classic/Bedrock support, gameplay
fixes, resources and Mixin set.

The maintained OpenZen delta is intentionally small:

- ViaVersion and ViaBackwards are upgraded to `4.10.0`, with ViaLoader
  `2.2.13`;
- the original VFP version model and UI include Minecraft 1.20.5/1.20.6
  protocol `766`;
- the three ViaBackwards hops from native protocol `763` to `766` are
  bundled and verified in the remapped JAR;
- `OpenZenViaProtocol` is a thin integration API that lets FantNEL force only
  its loopback endpoint to protocol `766`; VFP still owns selection, protocol
  translation and the Netty pipeline;
- two obsolete ViaVersion Mixins whose target classes no longer exist in 4.10
  are removed, while the rest of the original VFP feature set remains present.

Build and verify this subproject directly with:

```powershell
.\gradlew.bat build --no-parallel
```

The root compatibility task `fabricmodbuild` builds this jar together with the
main Mizulune Fabric jar and the frozen FantNEL Host. Staged output is written
to `../build/mod-dist/fabric/`.

---

## Upstream ViaFabricPlus 2.8.7 README
