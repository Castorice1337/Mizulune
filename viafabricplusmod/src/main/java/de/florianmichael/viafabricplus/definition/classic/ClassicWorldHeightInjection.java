    ...
    public static PacketHandler handleJoinGame(final PacketHandler parentHandler) {
        return wrapper -> {
            ...
            if (ProtocolHack.getTargetVersion(wrapper.user().getChannel()).isOlderThanOrEqualTo(VersionEnum.c0_28toc0_30)) {
                ...
                final CompoundTag dimensionType = registry.getCompoundTag("minecraft:dimension_type");
                final ListTag<CompoundTag> dimensions = dimensionType.getListTag("value", CompoundTag.class);
                for (CompoundTag dimension : dimensions) {
                    changeDimensionTagHeight(wrapper.user(), dimension.get("element"));
                }
